...
public class LawyerAccountsReviewActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.lawyer_accounts_review);
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			if (_data.get((int)_position).get("type").toString().equals("lawyer")) {
				name.setText(_data.get((int)_position).get("lawyer name").toString());
				status.setText(_data.get((int)_position).get("ac status").toString());
				linear1.setVisibility(View.VISIBLE);
			}
			button1.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					map.put("ac status", "accept");
					user.child(_data.get((int)_position).get("uid").toString()).updateChildren(map);
					lawyer.child(_data.get((int)_position).get("uid").toString()).updateChildren(map);
					map.clear();
				}
			});
